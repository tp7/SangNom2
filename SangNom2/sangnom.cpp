...
void SangNom2::prepareBuffers(const BYTE* pSrc, BYTE* pDst, int width, int height, int srcPitch, int dstPitch) {
    ...
    for (int y = 1; y < height / 2; y++) {
        for (int x = 0; x < width; x += 16) {
            auto cur_minus_3   = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pSrc+x-3)); 
            auto cur_minus_2   = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pSrc+x-2)); 
            auto cur_minus_1   = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pSrc+x-1)); 
            auto cur           = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pSrc+x));
            auto cur_plus_1    = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pSrc+x+1)); 
            auto cur_plus_2    = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pSrc+x+2)); 
            auto cur_plus_3    = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pSrc+x+3)); 

            auto next_minus_3  = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pSrcn2+x-3)); 
            auto next_minus_2  = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pSrcn2+x-2)); 
            auto next_minus_1  = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pSrcn2+x-1)); 
            auto next          = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pSrcn2+x)); 
            auto next_plus_1   = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pSrcn2+x+1)); 
            auto next_plus_2   = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pSrcn2+x+2)); 
            auto next_plus_3   = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pSrcn2+x+3)); 

            auto adiff_m3_p3 = simd_abs_diff_epu8(cur_minus_3, next_plus_3);
            _mm_storeu_si128(reinterpret_cast<__m128i*>(buffers[ADIFF_M3_P3]+bufferOffset+x), adiff_m3_p3);

            auto adiff_m2_p2 = simd_abs_diff_epu8(cur_minus_2, next_plus_2);
            _mm_storeu_si128(reinterpret_cast<__m128i*>(buffers[ADIFF_M2_P2]+bufferOffset+x), adiff_m2_p2);

            auto adiff_m1_p1 = simd_abs_diff_epu8(cur_minus_1, next_plus_1);
            _mm_storeu_si128(reinterpret_cast<__m128i*>(buffers[ADIFF_M1_P1]+bufferOffset+x), adiff_m1_p1);

            auto adiff_0       = simd_abs_diff_epu8(cur, next);
            _mm_storeu_si128(reinterpret_cast<__m128i*>(buffers[ADIFF_P0_M0]+bufferOffset+x), adiff_0);

            auto adiff_p1_m1  = simd_abs_diff_epu8(cur_plus_1, next_minus_1);
            _mm_storeu_si128(reinterpret_cast<__m128i*>(buffers[ADIFF_P1_M1]+bufferOffset+x), adiff_p1_m1);

            auto adiff_p2_m2  = simd_abs_diff_epu8(cur_plus_2, next_minus_2);
            _mm_storeu_si128(reinterpret_cast<__m128i*>(buffers[ADIFF_P2_M2]+bufferOffset+x), adiff_p2_m2);

            auto adiff_p3_m3  = simd_abs_diff_epu8(cur_plus_3, next_minus_3);
            _mm_storeu_si128(reinterpret_cast<__m128i*>(buffers[ADIFF_P3_M3]+bufferOffset+x), adiff_p3_m3);
            ...
            _mm_storeu_si128(reinterpret_cast<__m128i*>(buffers[SG_FORWARD]+bufferOffset+x), absdiff_p1_p2);
            //////////////////////////////////////////////////////////////////////////
            ...
            _mm_storeu_si128(reinterpret_cast<__m128i*>(buffers[SG_REVERSE]+bufferOffset+x), absdiff_p3_p4);
            //////////////////////////////////////////////////////////////////////////
        }
        pSrc += srcPitch*2;
        pSrcn2 += srcPitch*2;
    ...
    for (int i = 0; i < 9; ++i) {
        ...
        for (int y = 0; y < bufferHeight_ - 1; ++y) {
            for(int x = 0; x < bufferWidth_; x+= 16) {
                ...
                auto srcn = _mm_load_si128(reinterpret_cast<const __m128i*>(pSrcn+x));
                auto srcn2 = _mm_load_si128(reinterpret_cast<const __m128i*>(pSrcn2+x));
                ...
                _mm_store_si128(reinterpret_cast<__m128i*>(pDst+(x*2)), sum_lo);
                _mm_store_si128(reinterpret_cast<__m128i*>(pDst+(x*2)+16), sum_hi);
            }

            for (int x = 0; x < bufferWidth_; x+= 16) {
                auto cur_minus_6_lo = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pDst+x*2-6));
                auto cur_minus_4_lo = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pDst+x*2-4));
                auto cur_minus_2_lo = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pDst+x*2-2));
                auto cur_lo         = _mm_load_si128(reinterpret_cast<const __m128i*>(pDst+x*2));
                auto cur_plus_2_lo  = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pDst+x*2+2));
                auto cur_plus_4_lo  = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pDst+x*2+4));
                auto cur_plus_6_lo  = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pDst+x*2+6));

                auto cur_minus_6_hi = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pDst+x*2-6+16));
                auto cur_minus_4_hi = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pDst+x*2-4+16));
                auto cur_minus_2_hi = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pDst+x*2-2+16));
                auto cur_hi         = _mm_load_si128(reinterpret_cast<const __m128i*>(pDst+x*2+16));
                auto cur_plus_2_hi  = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pDst+x*2+2+16));
                auto cur_plus_4_hi  = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pDst+x*2+4+16));
                auto cur_plus_6_hi  = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pDst+x*2+6+16));
                ...
                _mm_store_si128(reinterpret_cast<__m128i*>(pSrcn+x), result);
            }
...
void SangNom2::processPlane(const BYTE* pSrc, BYTE* pDst, int width, int height, int srcPitch, int dstPitch) {
    ...
    for (int y = 1; y < height / 2; ++y) {
        for (int x = 0; x < width; x += 16) {
            auto buf0 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(buffers[ADIFF_M3_P3]+x+y*width)); 
            auto buf1 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(buffers[ADIFF_M2_P2]+x+y*width)); 
            auto buf2 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(buffers[ADIFF_M1_P1]+x+y*width)); 
            auto buf3 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(buffers[SG_FORWARD]+x+y*width)); 
            auto buf4 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(buffers[ADIFF_P0_M0]+x+y*width)); 
            auto buf5 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(buffers[SG_REVERSE]+x+y*width)); 
            auto buf6 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(buffers[ADIFF_P1_M1]+x+y*width)); 
            auto buf7 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(buffers[ADIFF_P2_M2]+x+y*width)); 
            auto buf8 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(buffers[ADIFF_P3_M3]+x+y*width)); 
            ...
            _mm_storeu_si128(reinterpret_cast<__m128i*>(pDst+x+dstPitch), acc);
        }
        pSrc += srcPitch * 2;
        pSrcn2 += srcPitch * 2;
        pDst += dstPitch *2;
